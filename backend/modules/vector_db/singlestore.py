"""
SingleStore DB designed for apps, analytics and gen AI,
SingleStore is the world's only real-time data platform that can
read, write and reason on petabyte-scale data in a few milliseconds.
"""
import json
...
class SSDB(SingleStoreDB):
    ...
    def add_texts(self,
                  ...
                  **kwargs) -> List[str]:
        """Добавление текстов в vectorstore.
        Args:
            texts (Iterable[str]): Итератор добавляемых strings/text
            metadatas (Optional[List[dict]], optional): Дополнительный список метаданных
            embeddings (Optional[List[List[float]]], optional): Предварительно созданные embeddings
        Returns:
...
class SingleStoreVectorDB(BaseVectorDB):
    ...
    def list_data_point_vectors(self,
                                collection_name: str,
                                data_source_fqn: str,
                                batch_size: int = BATCH_SIZE) -> List[DataPointVector]:
        logger.debug(

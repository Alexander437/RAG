...
def get_collections() -> Dict[str, List[Collection]]:
    try:
        ...
        raise HTTPException(status_code=500, detail={"collections": []})


@router.post("/")
def create_collection(collection: CreateCollectionDto) -> Dict[str, Collection]:
    try:
        ...
        logger.exception(exp)
        raise HTTPException(status_code=500, detail={"collection": None})
...
# Не работает
@router.post("/ingest")
async def ingest_data(request: IngestDataToCollectionDto) -> JSONResponse:
...
def list_data_ingestion_runs(request: ListDataIngestionRunsDto) -> Dict[str, list]:
...
def get_collection_status(
    ...
    if data_ingestion_run is None:
        raise HTTPException(
            ...
            detail={
                "status": None,
                "message": f"Data ingestion run {data_ingestion_run_name} not found"
            }
        )
